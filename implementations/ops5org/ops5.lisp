...
;
; File:         ops5.l
; Description:  Source code for OPS5 production language.
;               Was not developed at HP, but by Charles Forgy at at Carnegie-Mellon.
;
; (c) Copyright 1986, Hewlett-Packard Company, all rights reserved.
;
;  
;      Three minor changes have been make to OPS5 so far to
;      have it work on HP's Common LISP.  None of these changes 
;      were due to HP's Common LISP.
;
;      1.  Uncommented form on line 1030 that checks whether
;          ASSQ is a defined function.  Some Common Lisp implementations
;          include ASSQ as an extension, notably Spice Lisp and Zetalisp CLCP.
;          If ASSQ is not a defined function then a macro ASSQ is defined
;          that uses the standard Common Lisp function ASSOC.
;          Leaving the comment characters was a small oversight of the
;          programmers.
;
;      2.  The same type of problem as #1 above occurs on line 1063.
;          A form checking for a function named DELQ was left commented out.
;          This form defines a macro DELQ in terms of the standard
;          Common Lisp function DELETE.
;
;      3.  The macro definition of DELQ in #2 above did not correctly
;          evaluate the formal parameters appearing in a backquoted
;          expression.  Commas were placed before the formal parameters
...
;      Enhancements:
;      
;      1.  Added global switch (*print-statistics*) to allow the user the
;          option of turning off the printing of statistics that occurs
;          after the conflict set is emptied.  Previously, the statistics
;          were always printed. 
;      
;
;      2.  Exported all of the ops5 "keywords" so the ops5 package can be used.
;          A conflict exists between lisp:write, ops5:write and lisp:remove,
;          ops5:remove, respectively.  To avoid a continuable error and to give
;          the ops symbols precedence, evaluate the following:
;
;                (shadowing-import '(ops5:write ops5:remove))
;                (use-package 'ops5)
;
;      3.  Setup ops5 module with the Common LISP provide statement.
;          This means that if the OPS5 object file is placed in the
;          $LISP/modules/local directory, then (require "ops5") will
;          correctly load the ops5 module.
;
;      
;
;      Directions:
;
;      1.  To compile the ops5 source execute
;
;             (proclaim '(optimize (speed 3)))
;             (compile-file <path to the ops5 source>
;                           :output "$LISP/modules/local/ops5.b")
;
;          This puts the object file in the correct location so that to use ops5 all
;          that needs to be executed is
;
;             (require "ops5")
;      
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; -*- Mode: Lisp; Package: Ops5 -*-
(in-package 'ops5)

;;; Ops5 wants to use the names Remove and Write, so we shadow them in the Ops5
;;; package.

(shadow '(remove write) (find-package 'ops5))


(export '(accept acceptline back bind build call cbind closefile compute
		 crlf cs default excise external genatom halt in inf literal
		 literalize litval make matches modify openfile out p pbreak pm 
		 ppwm remove rjust run strategy substr tabto vector-attribute
		 watch wm write $assert $litbind $parameter $parametercount
		 $reset $tab $value $varbind <> <=> --> \\ >> <<))
		 
;;; OPS5 uses these CL symbols:  -  // <  <=  - > >= * +  ;;;
;;; ------------------------------------------------------------
...
;
;
;  Spice Lisp mods by Dario Giuse and Skef Wholey for Common Lisp on a Perq.
;
;  vax/vms common lisp mods by George Wood -- not completed
;
...
;
; 11/27/84 Skef Wholey
;  - Nuked the (proclaim '(localf ...)).

;
; 9/3/84 Skef Wholey
;  - Made this live in the OPS5 package.
;  - Cleaned up brutal package hackery so the Lisp stays kosher.
;  - Changed the == nonsense to use eql, the right Common Lisp thing.
;  - Nuked the so called "fast" -symeval in favor of symbol-value.
;  - Changed field-name to use a vector instead of being a huge cond.
;  - Cleaned up ASSQ and DELQ stuff so it doesn't crash 3600's.
;  These changes resulted in about a 30% speedup (using TTT as a benchmark).
;
; 6/22/84 Dario Giuse
...
;
; 6/20/84 Dario Giuse
;  - Completely rewritten ops-acceptline. This is now working properly in Spice
...
;    implementations). This should probably be done only for *standard-output*,
;    not for all files.
;
; 6/8/84 Jcp
...
;
;changes made by gdw
; changed macro to system::macro to access buried fn
...
; Added 4/24/86 by HP
; Default is to print the statistics when all the rules in the
; conflict set have been fired.  Setting *print-statistics* to non-nil
; turns it off.
(proclaim '(special *print-statistics*)) 
...
(defun cmp-number (test)   ;jgk inserted concatenate form
  (link-new-node (list (intern (concatenate 'string
					    ...
					    (symbol-name  test)
					    "N")
			       (find-package 'ops5))        ;@@@ error? reported by laird fix\
		       nil
		       (current-field)
		       (sublex)))) 
...
;
;  The following is the original OPS5 ops-accept function.
;  I modified it below so input from nmode would only require
;  one Lisp-E.  I think this might be a bug on their part.  I see
;  no need to do an peek-char on *standard-input*.
;
#|
(defun ops-accept (z)
  (prog (port arg)
    ...
    (cond ((= (length z) 1)
	   ...
	   (cond ((null port) 
		  ...
		  (return nil)))))
    (cond ((equal (peek-char t port nil "eof" t ) "eof" )
	   ($value 'end-of-file)
	   (return nil)))
...
(defun print-times (mess)
  (prog (cc)
    ...
    (when *print-statistics*                                ; added *print-statistics* on 4/24/86
      (terpri)                                              ;  to give user more control.  
      (princ mess)
      (terpri)
