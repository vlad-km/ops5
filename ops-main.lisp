...
(defun process-changes (adds dels)
  (prog (x)
   process-deletes
     (and (atom dels) (go process-adds))
     (setq x (car dels))
     (setq dels (cdr dels))
     (remove-from-wm x)
     (go process-deletes)
   process-adds
     (and (atom adds) (return nil))
     (setq x (car adds))
     (setq adds (cdr adds))
     (add-to-wm x nil)
     (go process-adds))) 
...
(defun main nil
  (prog (instance r)
     ...
     (setq instance nil)
   dil
     (setq *phase* 'conflict-resolution)
     (cond (*halt-flag*
...
(defun ops-watch (z)
  (cond ((equal z '(0.)) (setq *wtrace* nil *ptrace* nil) 0)
        ...
        ((equal z '(3.)) (setq *wtrace* t   *ptrace* t)
...
(defun matches3 (nodes ce part)
  (cond ((not (null nodes))
         ...
         (format t "~% ** matches for ~A ** " (list ce))
  ...
  (prog (al att ppdat)
   ...
   top
     (cond ((not (atom al))
            (setq att (car al))
            (setq al (cdr al))
            (setq ppdat (cons (cons (literal-binding-of att) att) ppdat))
            (go top)))
...
(defun note-user-assigns2 (att)
  (prog (num conf buck clash)
     ...
     (and clash
          (%warn '|attributes in a class assigned the same number| (cons att clash)))
     (return nil)))
  ...
  (prog (cs entry)
   ...
   l2
     ...
     (cond ((eq name (caar entry))
            ;; @vlad-km :test #'equal
            (setq *conflict-set* (delete entry *conflict-set* :test #'equal))
            (go l1))
           (t (go l2))))) 
